...
clubs = session.query(Club).join(Jugador).filter(Jugador.nombre.like("%Da%")).all()
...
registros = session.query(Club, Jugador).join(Jugador).filter(Jugador.nombre.like("%Da%")).all()
